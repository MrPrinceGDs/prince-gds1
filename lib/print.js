...
⏰ ${chalk.black(chalk.bgGreen(
      new Date((m.messageTimestamp || Date.now()/1000) * 1000)
        .toLocaleTimeString('en-EN', { timeZone: 'Asia/Karachi' })
    ))}
...
${!isSuperAdmin && isAdmin ? '👑 Admin  ' : ''}${isBotMessage ? '🤖 Bot Message  ' : ''}
...
`.trim())
  ...
  if (/document/i.test(messageType)) {
    console.log(`📄 ${content?.fileName || 'Document'}`)
  }
